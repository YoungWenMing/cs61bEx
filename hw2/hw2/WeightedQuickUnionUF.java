...
public class WeightedQuickUnionUF {
    ...
    public int find(int p) {
        validate(p);
        while (p != parent[p])      //find the component's identifier
            p = parent[p];
        return p;
    }
    ...
    public boolean connected(int p, int q) {
        return find(p) == find(q);          //the two are of the same component if their final identifier is same
    }
    ...
    public void union(int p, int q) {
        ...
        count--;            //number of components decreased by 1 after union.
    }
