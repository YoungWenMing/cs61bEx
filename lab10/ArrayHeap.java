...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        if(pIndex != 0 && min(index, pIndex) != pIndex){        // the parent's priority is smaller
            swap(index, pIndex);
            swim(pIndex);
        }
    }
    ...
    public T removeMin() {
        /* TODO: Your code here! */
        int rightMost = size();
        T minV = getNode(1).item();

        swap(rightMost, 1);

        contents[rightMost] = null;
        size = size -1;
        sink(1);
    ...
    public void testSink() {
        ...
        pq.changePriority("x2", 20.0);
        pq.changePriority("x3", 15.0);
